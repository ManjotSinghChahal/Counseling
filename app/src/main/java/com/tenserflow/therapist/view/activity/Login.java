...
import com.tenserflow.therapist.Webservice.PolicyUrl;
...
public class Login extends AppCompatActivity implements GoogleApiClient.OnConnectionFailedListener, GoogleApiClient.ConnectionCallbacks {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        //---------------remember me funct.-----------------------
        if (HelperPreferences.get(this).getString("rememberME")!=null && !HelperPreferences.get(this).getString("rememberME").equalsIgnoreCase("")
                && HelperPreferences.get(this).getString("rememberME").equalsIgnoreCase("checked"))
        {
            String username = HelperPreferences.get(this).getString("username");
            String password = HelperPreferences.get(this).getString("password");

            edtEmailSignin.setText(username);
            edtPasswordSignin.setText(password);
        }


        //---------------------policy url----------------------------
        if (!Global.TERMS_CONDITIONS.equalsIgnoreCase(""))
        {
            getPolicyUrl();
        }
        ...
        if (requestCode == RC_SIGN_IN) {
            ...
            if (result.isSuccess()) {
                ...
                if (!InternetConnectivity.isConnected(this)) {
                    ...
                    register_api.social_login(this, acct.getId(), "g", result.getSignInAccount().getGivenName(),result.getSignInAccount().getFamilyName(), acct.getEmail(), relTopSignin, photo_url);
                }
                        ...
                        try {
                           ...
                           /* Log.e("SDVrfdvcd", first_name);
                            ...
                            if (!InternetConnectivity.isConnected(Login.this)) {
                                ...
                                SocialLogin_Api register_api = new SocialLogin_Api();
                                register_api.social_login(context, id, "f", first_name,last_name, email, relTopSignin, image_url);
                            }
    ...
    private void getPolicyUrl()
    {
        PolicyUrl policyUrl = new PolicyUrl();
        policyUrl.policyUrl(this);
    }
}
