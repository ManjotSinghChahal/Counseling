...
public class Register extends AppCompatActivity implements GoogleApiClient.OnConnectionFailedListener, GoogleApiClient.ConnectionCallbacks {
    ...
    String country_code = "";
    @BindView(R.id.edt_nameLast_signup)
    EditText edtNameLastSignup;
...
//    String country_code="";
    ...
    public void onSignupClicked() {
        ...
        if (edtNameSignup.getText().toString().trim().equalsIgnoreCase("")) {
            ...
            Snackbar snackbar = Snackbar.make(relTopSignup, "Please enter first name", Snackbar.LENGTH_LONG);
            snackbar.show();
        }
        else if (edtNameLastSignup.getText().toString().trim().equalsIgnoreCase("")) {
            edtNameLastSignup.requestFocus();
            //  edtNameSignup.setError("Please enter name");
            Snackbar snackbar = Snackbar.make(relTopSignup, "Please enter last name", Snackbar.LENGTH_LONG);
            snackbar.show();
         }
            ...
            snackbar.show();
        } else if (edtPhoneSignup.getText().toString().trim().length() < 6 || edtPhoneCodeSignup.getText().toString().trim().length() > 15) {
            edtPhoneSignup.requestFocus();
          ...
          //  edtAddressSignup.setError("Please enter address");
            ...
            if (!InternetConnectivity.isConnected(this)) {
                ...
                register_api.register(this, edtNameSignup.getText().toString().trim(),edtNameLastSignup.getText().toString().trim(), edtEmailSignup.getText().toString().trim(), edtCPasswordSignup.getText().toString().trim(), edtPhoneSignup.getText().toString().trim(), "", date, relTopSignup, edtPhoneCodeSignup.getText().toString().trim());
            }
        }
        ...
        if (requestCode == RC_SIGN_IN) {
            ...
            if (result.isSuccess()) {
                ...
                if (acct.getPhotoUrl() == null) {
                    photo_url = "http://therapy.gangtask.com/public/images/default1.jpg";
                } else {
                ...
                if (!InternetConnectivity.isConnected(this)) {
                    ...
                    register_api.social_login(this, acct.getId(), "g", result.getSignInAccount().getGivenName(),result.getSignInAccount().getFamilyName(), acct.getEmail(), relTopSignup, photo_url);
                        ...
                        try {
                           ...
                           /* Log.e("SDVrfdvcd", first_name);
                            ...
                            if (!InternetConnectivity.isConnected(Register.this)) {
                                ...
                                SocialLogin_Api register_api = new SocialLogin_Api();
                                register_api.social_login(context, id, "f", first_name,last_name, email, relTopSignup, image_url);
                            }
